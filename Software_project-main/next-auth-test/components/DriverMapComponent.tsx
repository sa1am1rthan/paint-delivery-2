...
const MapComponent: React.FC = (role, state) => {
      ...
      try {
        ...
        if (Array.isArray(result.data)) {
          ...
          const pendingOnly = result.data.filter(
            (delivery: DeliveryData) => delivery.deliveryStatus !== 'delivered' && delivery.approval !== 'approved'
          );
  ...
  const handleStatusChange = async (value: string, record: any) => {
    try {
      ...
      if (response.ok) {
        // Update pendingDeliveries state
        setPendingDeliveries(prevDeliveries =>
          prevDeliveries.filter(delivery => 
            delivery.orderNumber !== record.orderNumber
          )
        );

        // Update optimizedRouteData state but keep the record
        ...
        // If status is changed to 'delivered', update the route but keep the markers
        if (value === 'delivered') {
          const updatedPendingDeliveries = pendingDeliveries.filter(
            delivery => delivery.orderNumber !== record.orderNumber
          );
          // Don't call getOptimizedRoute here to keep the route visible
        }
      } else {
      ...
      <div className="flex h-[calc(100vh-70px)]" style={{ marginTop: "70px" }}>
        ...
        <div className="flex-grow p-4 overflow-y-auto">
          ...
          <div>
            ...
            <Table 
              dataSource={Array.isArray(pendingDeliveries) ? pendingDeliveries : []} 
              columns={columns} 
              rowKey="orderNumber"
            />
            <h2 className="text-xl font-bold my-4">Optimized Route</h2>
            <Table 
              dataSource={Array.isArray(optimizedRouteData) ? optimizedRouteData : []} 
